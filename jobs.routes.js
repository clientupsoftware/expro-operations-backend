...
router.delete('/:id', requireRole('super'), async (req, res) => {
  ...
  try {
    ...
    // explosive_program_dispatches.job_id es NOT NULL - no se puede desvincular, se borra
    // directamente (el despacho a un Job no tiene sentido sin ese Job).
    await client.query('DELETE FROM explosive_program_dispatches WHERE job_id = $1', [id]);
